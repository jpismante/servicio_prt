	...
	<?php
		...
		$consulta_informe = select_informe($_GET['patente']);
		
		
		$numero_filas = mysqli_num_rows($consulta_informe);
...
<form action='guarda.php' method='post'>
	...
	<?php
		...
		while($usuario = $consulta_informe->fetch_assoc())
		{
...
// ------------------------------------------------------------------------------------
			if($detalle_pregunta!=$usuario['detalle_pregunta'])
			{
				$string = $string. "<tr>";
					...
					$string = $string. "<td colspan='2'>";
					if($usuario['valor_pregunta']=='1'){
							$string = $string.'APROBADO';
						}
					else if($usuario['valor_pregunta']=='0'){
							$string = $string.'RECHAZADO';	
					}else{
							$string = $string.'<b>'.$usuario['valor_pregunta'].'</b>';
						}
					$string = $string. "</td>" ;
				$string = $string. "</tr>";
			}
